...
#include "mapGen.h"
#include "trainer.h"
/* might only need dijkstra.h and mapGen.h */

/* defines ter_cost function for quickly accessing move_cost array */
#define ter_cost(x, y, c) move_cost[c][m->get_map_val(x, y)]

/* path structure used during path generation process */
typedef struct path {
    heap_node_t *hn;
    pair_t pos;
    uint8_t from[2];
...
/* min-heap comparison function for path generation */
static int32_t path_cmp(const void *key, const void *with) {
    return ((path_t *) key)->cost - ((path_t *) with)->cost;
}

/* min-heap comparison function for hiker distance map generation */
...
/* min-heap comparison function for rival distance map generation */
...
void dijkstra_path(Map *m, pair_t from, pair_t to) {
...
void pathfind(Map *m) {
    ...
    /* Set the starting point's cost to 0 */
    world.hiker_dist[world.player.get_pos_y()][world.player.get_pos_x()] = 
