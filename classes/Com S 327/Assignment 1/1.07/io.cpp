...
static int compare_trainer_distance(const void *v1, const void *v2) {
    // const Trainer *const *c1 = (const Trainer *const *) v1;
    // const Trainer *const *c2 = (const Trainer *const *) v2;
    Trainer *c1 = (Trainer *) v1;
    Trainer *c2 = (Trainer *) v2;
...
static Trainer *io_nearest_visible_trainer() {
    Trainer **c, *n;
    uint32_t x, y, count;

    c = (Trainer **) malloc(world.get_cur_map()->get_num_trainers() * sizeof (*c));
    ...
    for (y = 0; y < MAP_Y; y++) {
        for (x = 0; x < MAP_X; x++) {
            if (world.get_cur_map()->get_trainer(x, y)) {
                /* can't access ttype directly so we instead assume it based off the symbol */
                switch (world.get_cur_map()->get_trainer(x, y)->get_symbol()) {
                    case '@':
                    ...
                    case 's':
                    case 'n':
                        /* default case is to use color of tr_other */
                    default:
...
void io_battle(Trainer *aggressor, Trainer *defender) {
    NPC *npc;
    ...
    if (npc->get_ttype() == tr_hiker || npc->get_ttype() == tr_rival) {
        npc->set_mtype(move_wander);
    }
}

/* temporary placeholder for pokemon encounter display */
void io_poke_encounter() {
    ...
    mvprintw(0, 0, "Wild %s encountered!", p->get_name());
    mvprintw(22, 1, "Pokemon Info: Lvl. %d %s", p->get_level(), p->get_name());
    mvprintw(23, 1, "Pokemon Moveset: ");
    if (p->get_move(0)) {
        if (p->get_move(1)) {
            mvprintw(24, 8, "> %s (Lvl. %d)     > %s (Lvl. %d)", p->get_move(0)->name, p->get_move(0)->level,
                     p->get_move(1)->name, p->get_move(1)->level);
        } else {
            mvprintw(24, 8, "> %s (Lvl. %d)", p->get_move(0)->name, p->get_move(0)->level);
        }
    } else {
        mvprintw(24, 8, "This Pokemon has no valid moves!");
    }
    mvprintw(25, 1, "Pokemon Stats:   HP        Attack    Defense   Sp. Att   Sp. Def   Speed     ");
    mvprintw(26, 18, "%d", p->get_stat(stat_hp));
    mvprintw(26, 28, "%d", p->get_stat(stat_attack));
    mvprintw(26, 38, "%d", p->get_stat(stat_defense));
    mvprintw(26, 48, "%d", p->get_stat(stat_sp_attack));
    mvprintw(26, 58, "%d", p->get_stat(stat_sp_defense));
    mvprintw(26, 68, "%d", p->get_stat(stat_speed));
    mvprintw(27, 1, "Pokemon IV:");
    mvprintw(27, 18, "%d", p->get_iv(stat_hp));
    mvprintw(27, 28, "%d", p->get_iv(stat_attack));
    mvprintw(27, 38, "%d", p->get_iv(stat_defense));
    mvprintw(27, 48, "%d", p->get_iv(stat_sp_attack));
    mvprintw(27, 58, "%d", p->get_iv(stat_sp_defense));
    mvprintw(27, 68, "%d", p->get_iv(stat_speed));
    mvprintw(28, 1, "Gender: %s", p->get_gender() == 'm' ? "Male" : "Female");
    mvprintw(29, 1, "Shiny: %s", p->is_shiny ? "Yes" : "No");
    refresh();
    getch();
}
...
uint32_t move_pc_dir(uint32_t input, pair_t dest) {
    ...
    switch (input) {
        ...
        case 9:
            ...
            break;
        case '>':
            if (world.get_cur_map()->get_map_val(world.player.get_pos_x(), world.player.get_pos_y()) == ter_mart) {
                io_pokemart();
            } else if (world.get_cur_map()->get_map_val(world.player.get_pos_x(), world.player.get_pos_y()) == ter_center) {
                io_pokemon_center();
            }
            break;
    }
    ...
    /* check if the destination space is occupied by an NPC */
    if (world.get_cur_map()->get_trainer(dest)) {
        if (dynamic_cast<NPC *>(world.get_cur_map()->get_trainer(dest)) && ((NPC *) world.get_cur_map()->get_trainer(dest))->is_defeated) {
            // some kind of greeting here would be nice
            return 1;
        } else if (dynamic_cast<NPC *>(world.get_cur_map()->get_trainer(dest))) {
            io_battle(&world.player, world.get_cur_map()->get_trainer(dest));
            // not actually moving, so set dest back to the PC position
            dest[dim_x] = world.player.get_pos_x();
            dest[dim_y] = world.player.get_pos_y();
            /* return here to prevent having to check if a battle occurred when trying to spawn a pokemon */
            return 0;
        }
    }
    ...
    /* check if a pokemon should spawn */
    if (world.get_cur_map()->get_map_val(dest) == ter_grass) {
        if (rand() % 100 < POKEMON_SPAWN_PROB) {
            io_poke_encounter();
        }
    }
