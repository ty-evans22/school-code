...
int32_t trainer_cmp(const void *key, const void *with) {
    return ((trainer_t *) key)->get_curr_time() - ((trainer_t *) with)->get_curr_time();
}
...
/* randomly sets the direction for an npc */
void rand_dir(npc_t *t) {
  int _i = rand() & 0x7;
  t->set_dir(all_dirs[_i][0], all_dirs[_i][0]);
}
...
void new_tr_other() {
    ...
    switch (rand() % 4) {
        case 0:
            t = new npc_t(move_pace);
            break;
        case 1:
            t = new npc_t(move_wander);
            break;
        case 2:
            t = new npc_t(move_sentry);
            break;
        case 3:
            t = new npc_t(move_walk);
            break;
    }
    t->set_pos(pos);
    rand_dir(t);
...
void place_pc() {
    ...
    world.get_cur_map()->set_trainer(*world.player.get_pos(), &world.player);
    pc_t *p = dynamic_cast<pc_t *>(&world.player);

    if ((c = (pc_t*)heap_peek_min(&world.get_cur_map()->queue))) {
        p->set_curr_time(c->get_curr_time());
    } else {
        p->set_curr_time(0);
    }
}
...
int new_map(int teleport) {
    ...
    map_t *m = new map_t();
    world.set_map(*world.get_cur_idx(), m);
    world.set_cur_map(world.get_map(*world.get_cur_idx()));
    ...
    if (teleport) {
        ...
        } while (world.get_cur_map()->get_trainer(*world.player.get_pos()) ||
                (move_cost[tr_pc][world.get_cur_map()->get_map_val(*world.player.get_pos())] == INT_MAX) ||
